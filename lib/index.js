  ...
  _createClass(DigitString, {
    validate: {
      value: function validate(input) {
        ...
        if (!_.isUndefined(this.props.maxLength) && input.length > this.props.maxLength || !_.isUndefined(this.props.minLength) && input.length < this.props.minLength) {
          return false;
        }

        var intValue = parseInt(input, 10);
        if (isNaN(intValue) || !_.isUndefined(this.props.max) && intValue > this.props.max || !_.isUndefined(this.props.min) && intValue < this.props.min) {
          return false;
        }
